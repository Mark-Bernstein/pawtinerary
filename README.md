...
The browser stores `{ version: 3, dogs, services }` under `pawtinerary.data.v3` in `localStorage`. Existing version 1 and version 2 records are loaded automatically and saved in version 3 format. Changes persist automatically after add, edit, completion, cancellation, or deletion. The selected language is stored separately under `pawtinerary.language.v1` and defaults to English. The selected theme is stored under `pawtinerary.theme.v1` and defaults to Dog Mode. Light Mode keeps the original palette; Dog Mode adds a warm palette, paw and bone pattern, and playful card styling. The schedule view selection uses `sessionStorage`.

Each service belongs to one dog, date, and group, and can be scheduled, completed, or cancelled. The Add Service dialog and dog form let you tap multiple calendar dates for a group, and tap a selected date again to remove it. Adding several dates in the dialog creates all their services together. The Today schedule shows a numbered list of dogs with scheduled services under each group heading, above the detailed cards. A dog's Rate is charged once per service, so services in two groups create two charges. Completed services preserve the Rate in effect when they were completed; scheduled services use the dog's current Rate. Deleting a dog also deletes every associated service and its earnings.
